# ═══════════════════════════════════════════════════════════════════════════
#  Sri Lanka Weather Forecasting: Data Preprocessing and EDA
#  Complete preprocessing and exploratory analysis before SARIMA modeling
# ═══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# 4. DATA QUALITY ASSESSMENT AND SUMMARY
# ══════════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════════
# 5. SARIMA MODEL FITTING AND FORECASTING
# ══════════════════════════════════════════════════════════════════════════
...
forecast_district <- function(district_name, h = 48) {
...
for (district in unique(weather_monthly$city_name)) {
  result <- forecast_district(district, h = 48)
  if (!is.null(result)) {
...
ljung_box_results <- data.frame(
  District = character(),
  Temp_p_value = numeric(),
  Rain_p_value = numeric(),
  Temp_Autocorrelation = character(),
  Rain_Autocorrelation = character(),
  stringsAsFactors = FALSE
)

# Loop through forecast results and compute Ljung-Box p-values
for (district in names(forecast_results)) {
  temp_model <- forecast_results[[district]]$temp_model
  rain_model <- forecast_results[[district]]$rain_model
  
  # Perform Ljung-Box tests
  temp_lb <- Box.test(residuals(temp_model), lag = 20, type = "Ljung-Box", fitdf = length(temp_model$coef))
  rain_lb <- Box.test(residuals(rain_model), lag = 20, type = "Ljung-Box", fitdf = length(rain_model$coef))
  
  ljung_box_results <- rbind(
    ljung_box_results,
    data.frame(
      District = district,
      Temp_p_value = temp_lb$p.value,
      Rain_p_value = rain_lb$p.value,
      Temp_Autocorrelation = ifelse(temp_lb$p.value < 0.05, "Minor Autocorrelation", "None"),
      Rain_Autocorrelation = ifelse(rain_lb$p.value < 0.05, "Minor Autocorrelation", "None")
    )
  )
}

# Save to CSV
write.csv(ljung_box_results, "Forecast_Plots/Ljung_Box_Test_Summary.csv", row.names = FALSE)
